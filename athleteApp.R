...
load_benchmarks <- function(path) {
  # Wrap the path in 'file()' so R knows to look on your disk
  raw <- fromJSON(file(path), simplifyDataFrame = FALSE)
...
server <- function(input, output, session) {
  ...
  # 3. Inject the "Live User" into the Dropdown on load
  observe({
    q <- live_query()
    if (!is.null(q$user)) {
      all_usernames <- c(q$user, athletes$username)
      all_display   <- c(paste(q$user, "(Live Data)"), athletes$display_name)
      updateSelectInput(session, "athlete_sel", 
                        choices = setNames(all_usernames, all_display),
                        selected = q$user)
    }
  })
  
  # 4. Use the Dropdown to dictate the data!
  ath <- reactive({
    q <- live_query()
    if (!is.null(q$user) && input$athlete_sel == q$user) {
      return(live_profile()) # Use Live Data
    } else {
      return(athletes %>% filter(username == input$athlete_sel)) # Use Pre-saved Player
    }
  })
  ...
  ath_his <- reactive({
    ...
    if (!is.null(q$user) && input$athlete_sel == q$user) {
      df <- live_profile()
      df$test_date <- Sys.Date()
      return(df)
    } else {
      return(history %>% filter(username == input$athlete_sel) %>% arrange(test_date))
    }
  })
